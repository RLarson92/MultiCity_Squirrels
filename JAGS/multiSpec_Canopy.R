...
model{
  ...
  for(j in 1:nsite){
    ...
    # natural parameters
    # 1 = fox, 2 = gray, 3 = red
    f1[j,1] <- a0[city_vec[j]] + a1[city_vec[j]]*canopy[j]
    f2[j,1] <- b0[city_vec[j]] + b1[city_vec[j]]*canopy[j]
    f3[j,1] <- c0[city_vec[j]] + c1[city_vec[j]]*canopy[j]
    f12[j,1] <- d0[city_vec[j]] + d1[city_vec[j]]*canopy[j]
    f13[j,1] <- e0[city_vec[j]] + e1[city_vec[j]]*canopy[j]
    f23[j,1] <- g0[city_vec[j]] + g1[city_vec[j]]*canopy[j]
    ...
    for(t in 2:nseason){
      f1[j,t] <- a0[city_vec[j]] + a1[city_vec[j]]*canopy[j] + phi[1,city_vec[j]]*Xcat[x[j,t-1],1]
      f2[j,t] <- b0[city_vec[j]] + b1[city_vec[j]]*canopy[j] + phi[2,city_vec[j]]*Xcat[x[j,t-1],2]
      f3[j,t] <- c0[city_vec[j]] + c1[city_vec[j]]*canopy[j] + phi[3,city_vec[j]]*Xcat[x[j,t-1],3]
      f12[j,t] <- d0[city_vec[j]] + d1[city_vec[j]]*canopy[j]
      f13[j,t] <- e0[city_vec[j]] + e1[city_vec[j]]*canopy[j]
      f23[j,t] <- g0[city_vec[j]] + g1[city_vec[j]]*canopy[j]
